...
# make 6 day lag instead of one day
train <- train %>% mutate(Chla_ARlag_timestep_sqrt = lag(Chla_sqrt, 6L))
days <- seq(as.Date('2018-08-15'), as.Date('2019-12-22'), by = '6 days')
...
write.csv(train, paste0(folder, '/data_arima_6day_through_2019.csv'), row.names = FALSE)
