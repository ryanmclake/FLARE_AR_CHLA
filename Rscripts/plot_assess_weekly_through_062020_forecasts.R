...
for(i in 1:max_timestep){
  ...
  png(paste0(forecast_folder, '/figures/', timestep, '_Forecast_Timestep', i, '_moving_y_axis.png'), width = 1100, height = 800)
...
for(i in 1:max_timestep){
  ...
  #########################################################################
  png(paste0(forecast_folder, '/figures/', timestep, '_Forecast_Day', i, '.png'), width = 1100, height = 800)
  print(ggplot(temp, aes(forecast_date, forecast_mean_chl)) +
          ylim(0,150) +
          geom_line(size = 2) +
          geom_point(aes(forecast_date, obs_chl_EXO), size = 4, stroke = 0, shape = 19, color = 'dodgerblue4') +
