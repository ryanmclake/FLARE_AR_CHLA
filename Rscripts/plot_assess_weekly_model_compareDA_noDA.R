...
temp <- temp[temp$forecast_run_day >= '2019-06-01' ,]
temp <- temp[temp$forecast_run_day <= '2019-10-10',]

#create and save figure with forecast mean, confidence intervals, and obs chl
png(paste0('C:/Users/wwoel/Dropbox/Thesis/Figures/arima/no_DA_figs/', i, 'DAsim_no_DAdates_moving_y_axis.png'), width = 1100, height = 800)
...
for (i in 1:2) {
  ...
  temp_null <- temp_null[temp_null$date >= '2019-06-01' ,]
  temp_null <- temp_null[temp_null$date <= '2019-10-10',]
...
write.csv(metrics_overtime_weekly, paste0(folder, '/FCR_forecasts/weekly/no_DA', '/ForecastMetrics_Weekly_withDA.csv'), row.names = FALSE)
