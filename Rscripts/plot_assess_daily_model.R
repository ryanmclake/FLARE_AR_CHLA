...
# create table that null and forecast metrics will be writte into
...
metrics_overtime <- array(NA, dim = c(16,17))
...
for (i in 1:16) {
  ...
  datamerge_nonbloom <- temp_null[temp_null$forecast_run_day< as.Date("2019-07-10") | temp_null$forecast_run_day > as.Date("2019-08-10"), ]
...
weekly <- read.csv(paste0(folder, '/FCR_forecasts/weekly/weekly_05Feb2020/ForecastMetrics_Weekly.csv'))
weekly <- weekly[,-1]

metrics_overtime[7,10] <- weekly[1,3] #week1 forecast rmse
metrics_overtime[7,11] <- weekly[1,7] #week1 forecast r2
metrics_overtime[7,12] <- weekly[1,5] #week1 nonbloom rmse
metrics_overtime[7,13] <- weekly[1,9] #week1 r2 nonbloom

metrics_overtime[14,10] <- weekly[2,3] #week2 forecast rmse
metrics_overtime[14,11] <- weekly[2,7] #week2 forecast r2
metrics_overtime[14,12] <- weekly[2,5] #week2 nonbloom rmse
metrics_overtime[14,13] <- weekly[2,9]  #week2 r2 nonbloom

metrics_overtime[7,14] <- weekly[1,2]#week1 null rmse
metrics_overtime[7,15] <- weekly[1,6] #week1 null r2
metrics_overtime[7,16] <- weekly[1,4]#week1 nonbloom null rmse
metrics_overtime[7,17] <- weekly[1,8] #week1 r2 nonbloom null

metrics_overtime[14,14] <- weekly[2,2] #week2 null rmse
metrics_overtime[14,15] <- weekly[2,6]  #week2 null r2
metrics_overtime[14,16] <- weekly[2,4]  #week2 nonbloom rmse
metrics_overtime[14,17] <- weekly[2,8]  #week2 r2 nonbloom
