...
print('sw forecast pass')
  ...
  print('clean working directory pass')
  ###############################################
  ...
  print('source pass')
#  cleaned_inflow_file <- paste0(folder, "/sim_files/FCRinflow_postQAQC.csv")
...
#                             output_tz = reference_tzone)
  ...
  Discharge <- read.csv(paste0(folder,"/", "sim_files/FCRinflow_postQAQC.csv"))
#  Discharge <- read.csv(paste0(folder,"/", "ARIMA_working/FCR_inflow.csv")) #5-year average of inflow as 'forecast'
  ...
  discharge_forecast <- Discharge[date(Discharge$time)==date(forecast_start_day) | date(Discharge$time)==date(forecast_start_day) | date(Discharge$time)==date(forecast_start_day),]
  discharge_forecast[2:3,] <- discharge_forecast[1,] # repeat the first row for rows 2 and 3 because of the format of the dataframe below
   # with the lag, this is actually not a forecast but simply the observed discharge on the day of the forecast
  print('extract discharge data pass')
  ############################################
...
#                           window_length = window_length) # in days, this is set in the automated_forecast_ARIMA script
  ...
  load("C:/Users/wwoel/Desktop/FLARE_AR_CHLA/MCMC_output_ARIMA_Whitney_lag.Rdata")
  ...
  for(i in 2:3){
    ...
    for(j in 1:nmembers){  #want this to be length(nmembers) = 
      ...
      if(driver_uncertainty_discharge == TRUE){
        curr_discharge = rnorm(1, discharge_forecast[i,2], 0.004) # observational uncertainty with SD derived from residuals of WVWA vs Diana lm
      }else{
...
#  conf1 <- quantile(x[2, , ], c(0.025, 0.975))
 # conf2 <- quantile(x[3, , ], c(0.025, 0.975))
  ...
  error2_upper <- (error2_upper^2)/0.55 + 0.0308
  error2_lower <- qnorm(0.025, mean = mean(x[3,,]), sd = sd(x[3,,]))
  error2_lower <- (error2_lower^2)/0.55 + 0.0308
