...
for(i in 1:14){
  ...
  print(ggplot(temp, aes(forecast_date, forecast_mean_chl)) +
    geom_line(size = 2) +
    geom_text(x = as.Date('2020-06-10'), y = max(temp$forecast_CI95_upper, na.rm = TRUE), size = 15, label = paste0('RMSE = ', round(metrics_overtime[i,3], digits = 1))) + 
...
print(ggplot(temp, aes(date, mean)) +
        ...
        geom_point(aes(date, chla_ugL), size = 4, stroke = 0, shape = 19, color = 'dodgerblue4') +
