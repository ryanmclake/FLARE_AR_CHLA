...
catdata <- catdata %>% select(TIMESTAMP, Chla_1)
catdata$date <- date(catdata$TIMESTAMP)
catdata <- catdata %>% group_by(date) %>% mutate(mean_chl = mean(Chla_1, na.rm = TRUE))
mean_chl <- catdata[!duplicated(catdata$date),]
mean_chl <- mean_chl %>% mutate(diff = Chla_1 - mean_chl)
