...
#  }
 ...
 if( uncert_mode == 1){ # all sources of uncertainty are turned on aka normal run
  ...
  ###########################################################
  nsteps <- 8 # number of timesteps: initial, day 2, 4, 6, 8, 10, 12, 14
  ...
  sw_forecast <- data[c(2,4,6,8,10,12,14),]  
...
#                             output_tz = reference_tzone)
  ...
  forecast_sequence <- seq(as.Date(forecast_start_day), as.Date(forecast_start_day)+14, by = '2 days') 
...
#########################################################################################################################
  ...
  jags <- jags.model('jags_model.bug',
                     data = list('chla' = data$Chla_sqrt,
                                 'chla_lag' = data$Chla_ARlag2_sqrt,
