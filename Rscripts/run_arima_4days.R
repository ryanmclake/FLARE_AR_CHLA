...
#  }
 ...
 if( uncert_mode == 1){ # all sources of uncertainty are turned on aka normal run
  ...
  ###########################################################
  nsteps <- 4 # number of timesteps: initial, day 4, 8, 12
  # want to include the initial conditions in the nsteps because we will store this data later
  ...
  # extract all ensemble members for forecasts on day 4, 8, and 12 to get multiples of our 4-day timestep
  sw_forecast <- data[c(4,8,12),]  
...
#                             output_tz = reference_tzone)
  ...
  forecast_sequence <- seq(as.Date(forecast_start_day), as.Date(forecast_start_day)+12, by = '4 days') 
  ...
  # chla_obs is the matrix to pull from which will be used as the AR term for day 7 and day 14
...
#########################################################################################################################
  ...
  data <- read.csv(paste0(folder, '/data_arima_4day_through_2019.csv'))
  ...
  print('data subsetted to forecast start day')
    ...
    print('making training data only dataset')
  }
  library(rjags)
  ...
  jags <- jags.model('jags_model.bug',
                     data = list('chla' = data$Chla_sqrt,
                                 'chla_lag' = data$Chla_ARlag4_sqrt,
  ...
  # the model!
    ...
    for(j in 1:nmembers){  #want this to be length(nmembers) = 
      ...
      if(weather_uncertainty == TRUE){
        curr_shortwave = sw_forecast[i-1,met_index]
      }else{
