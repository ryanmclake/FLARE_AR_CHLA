...
# make timestep lag 
train <- train %>% mutate(Chla_ARlag_timestep_sqrt = lag(Chla_sqrt, timestep_numeric))
days <- seq(as.Date('2018-08-15'), as.Date('2019-12-22'), by = timestep_numeric)
...
write.csv(train, paste0(folder, '/data_arima_', timestep_numeric, 'day_through_2019.csv'), row.names = FALSE)
