# function to read in NOAA forecasts and write a csv for each ensemble member
# modified from function of same name in FlareR package
...
generate_glm_met_files <- function(obs_met_file = NULL,
  ...
  start_datetime <- lubridate::as_datetime(start_day)
  end_datetime <- lubridate::as_datetime(start_datetime + 16*60*60*24)
  
  full_time <- seq(start_datetime, end_datetime, by = "1 hour")
  full_time_hist <- full_time #seq(start_datetime, forecast_start_datetime - lubridate::hours(1), by = "1 hour")
  ...
  if(!is.null(obs_met_file) & !is.null(full_time)){
    ...
    for(i in 1:length(glm_met_vars)){
      
      met <- cbind(met, ncdf4::ncvar_get(obs_met_nc, glm_met_vars[i]))
    }
  ...
  return(filenames = gsub("\\\\", "/", filenames))
}