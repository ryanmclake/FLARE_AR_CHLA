...
for (i in 1:max_timestep) { 
  ...
  write.csv(dataset_null, paste0(null_folder, '/day_', i, '_null.csv'), row.names = FALSE)
...
for (i in 1:max_timestep) {
  ...
  obs <- read.csv(paste0(folder, '/obs_chl_exo.csv'))
...
for(i in 1:max_timestep){
  ...
  print(ggplot(temp, aes(forecast_date, forecast_mean_chl)) +
    geom_line(size = 2) +
      ...
      geom_point(aes(forecast_date, obs_chl_EXO), size = 4, stroke = 0, shape = 19, color = 'darkorchid4') +
...
temp <- read.csv(paste0(null_folder, '/day_1', '_null.csv'))
temp$date <- as.Date(temp$date)

png(paste0(forecast_folder, '/', timestep, '_Null', '.png'), width = 1100, height = 800)
print(ggplot(temp, aes(date, mean)) +
        ...
        geom_point(aes(date, chla_ugL), size = 4, stroke = 0, shape = 19, color = 'darkorchid4') +
