# run_arima function to produce daily chl forecasts using weekly model drivers (SW radiation and discharge)
...
train_data <- 'data_arima_daily_SWdischarge.csv'
...
source(paste0(folder,"/","Rscripts/data_assimilation_AR.R"))
  ...
  save(samples, file = "MCMC_output_ARIMA_highfrequency_SWdischarge.Rdata")

###########################################################################################
### create and archive null model #########################################################
###########################################################################################
  if(null_model){
