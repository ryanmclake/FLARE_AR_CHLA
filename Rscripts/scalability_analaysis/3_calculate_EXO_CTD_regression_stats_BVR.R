# calculate regression between CTD and EXO at Beaverdam Reservoir

###### CTD data, taken from published ctd data at BVR
...
ctd <- ctd %>% 
  dplyr::select(Reservoir, Site, Date, Depth_m, Chla_ugL) %>%
...
exo_daily <- exo %>% distinct(Day, .keep_all = TRUE) %>% 
  dplyr::select(Day, EXOChla_daily_mean)
