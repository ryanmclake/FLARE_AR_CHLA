...
download.file("https://raw.githubusercontent.com/CareyLabVT/ManualDownloadsSCCData/master/BVRplatform_manual_2020.csv", "./SCCData/bvre-data/BVRplatform_manual_2020.csv")
download.file('https://github.com/FLARE-forecast/BVRE-data/blob/bvre-platform-data/bvre-waterquality.csv?raw=true', "./SCCData/bvre-data/bvre-waterquality.csv") 
download.file('https://github.com/FLARE-forecast/BVRE-data/blob/bvre-platform-data/BVRplatform.csv?raw=true', "./SCCData/bvre-data/BVRplatform.csv")

bvrheader1<-read.csv("./SCCData/bvre-data/BVRplatform.csv", skip=1, as.is=T) #get header minus wonky Campbell rows
bvrdata1<-read.csv("./SCCData/bvre-data/BVRplatform.csv", skip=4, header=F) #get data minus wonky Campbell rows
...
bvrd<-read.csv("./SCCData/bvre-data/BVRplatform_manual_2020.csv") #read in manual data before gateway set up
...
#current BVR data with turbidity and TSS 
bvrheader5<-read.csv("./SCCData/bvre-data/bvre-waterquality.csv", skip=1, as.is=T) #get header minus wonky Campbell rows
bvrdata5<-read.csv("./SCCData/bvre-data/bvre-waterquality.csv", skip=4, header=F) #get data minus wonky Campbell rows
names(bvrdata5)<-names(bvrheader5) 


# manual downloads when data were not streaming come from: https://github.com/CareyLabVT/ManualDownloadsSCCData/tree/master/BVRPlatform
mydir = "./SCCData/bvre-data/Manual_Downloads"
...
myfilesBVR <- myfilesBVR[-c(9, 10, 11, 21, 24)]
...
for(k in 1:length(myfilesBVR)){
  ...
  if(ncol(bvrdata2) < 45){
    bvrdata2$Turbidity_FNU_1 <- NA
    bvrdata2$TSS_mgL_1 <- NA
    bvrdata2 <- bvrdata2[,names(bvrdata5)]
...
bvrdata_new=bvrdata_new%>%
  ...
  filter(TIMESTAMP!="")
bvrdata_new$TIMESTAMP <- as.POSIXct(bvrdata_new$TIMESTAMP)

plot(bvrdata_new$TIMESTAMP, bvrdata_new$Chla_1)

write.csv(bvrdata_new, "./SCCData/bvre-data/bvre-waterquality_2020-06-18_2021-10-11.csv", row.names = F)
