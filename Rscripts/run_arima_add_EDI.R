...
run_arima <- function(
  ...
  source(paste0(folder,"/","Rscripts/create_obs_met_input_with_EDI.R"))
 ...
 if( uncert_mode == 1){ # all sources of uncertainty are turned on aka normal run
  ...
  forecast_base_name <- paste0(year(forecast_start_day),
                               ...
                               forecast_day,
                               "gep_all_00z")
  temp_obs_fname_wdir <- temp_obs_fname  #paste0(working_glm, "/", temp_obs_fname)
  met_obs_fname_wdir <- met_obs_fname #paste0(met_station_location, "/", met_obs_fname)
  ...
  mising_met <-  create_obs_met_input_with_EDI(fname = met_obs_fname_wdir,
                       outfile=obs_met_outfile,
                       full_time_hour_local = full_time_hour_obs, 
                       input_file_tz = "EST5EDT", 
                       local_tzone = reference_tzone)
 #missing_met <- read.csv(obs_met_outfile)
    ...
    temp_met_file<- process_downscale_GEFS(folder,
                                           noaa_location,
...
#  conf1 <- quantile(x[2, , ], c(0.025, 0.975))
 # conf2 <- quantile(x[3, , ], c(0.025, 0.975))
  ...
  if(!is.na(x[1,1,])){
    ...
    plot(x_axis, ((x[,1,]^2)/0.55 +0.0308), type = 'o',ylim = range(c((min(out[1,8], out[2,8])), max(out[1,7], out[2,7], out[1,10], out[2,10]))) # once catwalk data cleaning script is running, can change this to include: out[1,10], out[2,10]
