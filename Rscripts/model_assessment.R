# function to calculate a list of model assessment metrics

model_metrics <- function(sim, obs){
  
  library(Metrics)
  library(hydroGOF)
  
  residual <- sim - obs
  data_minusglobamean <- obs - mean(obs, na.rm = TRUE)
  
  RMSE <- rmse(sim, obs)
  NSE <- NSE(sim, obs)
  KGE <- KGE(sim, obs)
  bias <- mean(residual, na.rm = TRUE)  #mean residual
  bias_sd <- sd(sim, na.rm = TRUE)/sd(obs, na.rm = TRUE)  # bias calculations for mean and median of both weeks (1 is non-biased, <1 means model is smoothing, >1 means model is predicting more fluctuations than are observed)
  R2_1_1 <- 1 - sum(residual^2)/sum(data_minusglobamean^2)
  
  x <- summary(lm(sim ~ obs))
  coeff_determination <- x$r.squared #r2 from linear model output
  
  cor_p <- cor(sim, obs, method = 'pearson') #pearson's correlation coefficient
  
  output <- data.frame(RMSE,NSE,KGE, bias, bias_sd, R2_1_1, coeff_determination, cor_p)
  return(output)
}




