...
run_arima <- function(
 ...
 if( uncert_mode == 1){ # all sources of uncertainty are turned on aka normal run
  ...
  begin_sim  <- lubridate::as_datetime(start_day,tz = reference_tzone)
  ...
  if(lubridate::day(forecast_start_time) < 10){
    forecast_day <- paste0("0", lubridate::day(forecast_start_time))
  }else{
    forecast_day <- paste0(lubridate::day(forecast_start_time))
  }
  if(lubridate::month(forecast_start_time) < 10){
    forecast_month <- paste0("0", lubridate::month(forecast_start_time))
  }else{
    forecast_month <- paste0(lubridate::month(forecast_start_time))
  }
  full_time <- seq(begin_sim, end_sim, by = "1 day")
  full_time_local <- lubridate::with_tz(full_time, tzone = local_tzone)
  ...
  forecast_base_name <- paste0(lubridate::year(forecast_start_day),
                               forecast_month,
                               forecast_day,
                               "_gep_all_00z")
 ...
 #  }
  ...
  discharge_forecast <- read.csv(file.path(folder, 'SCCData', 'bvre-data', 'BVR_flow_calcs_new.txt'))

  discharge_forecast <- discharge_forecast %>% 
    mutate(FLOW = discharge_forecast$Q_BVR_m3.d/24/60/60) %>% 
    select(time, FLOW)
...
# }
  ...
  cleaned_temp_oxy_chla_file <- paste0(working_arima, "/bvre-wq_postQAQC.csv")
  temp_oxy_chla_qaqc(data_file = temp_obs_fname_wdir[1], 
                     maintenance_file = paste0(data_location, '/bvre-data/BVR_maintenance_log.txt'), 
