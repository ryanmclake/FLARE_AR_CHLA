...
# DO NOT NEED TO RUN THE BELOW CODE
# IT IS USED TO WRANGLE DATA FROM SEPARATE YEARS WHICH IS NOW PUBLISHED IN ONE DATASET ON EDI, FluoroProbe.csv

##load in fluoro all data for 2014-2017
#fluoro14 <- read.csv("./historical_model_selection/Data/Fluoroprobe/FCR2014_FluoroMeta.csv")
#fluoro15 <- read.csv("./historical_model_selection/Data/Fluoroprobe/FCR2015_FluoroMeta.csv")
#fluoro16 <- read.csv("./historical_model_selection/Data/Fluoroprobe/FCR2016_FluoroMeta50.csv")
#fluoro17 <- read.delim("./historical_model_selection/Data/Fluoroprobe/FP_recal_2017.txt")  #fluoroprobe
#
...
#fluoro14 <- fluoro14[,c("Date", "Site", "Depth_m", "Total_chlorophyll", "Green_algae", "Cyanobacteria", "Diatoms","Cryptophyta")]
#fluoro15$Site <-  rep(50,nrow(fluoro15))
#fluoro15 <- fluoro15[,c("Date", "Site","Depth_m", "Total_chlorophyll", "Green_algae","Cyanobacteria","Diatoms","Cryptophytes")]
#colnames(fluoro15) <- colnames(fluoro14)
#fluoro16$Site <-  rep(50,nrow(fluoro16))
#fluoro16 <- fluoro16[,c("Date", "Site", "Depth_m", "Total_chlorophyll", "Green_algae", "Cyanobacteria", "Diatoms","Cryptophyta")]
#
...
#fluoro17 <- select(fluoro17, datetime, Site, Depth_m_m, total_conc_ugL, green_ugL, cyano_ugL, 
...
fluoro <- read.csv('./historical_model_selection/Data/Fluoroprobe/FluoroProbe.csv')
fluoro$Date <- date(fluoro$DateTime)

#select fluoro data which most closely matches water chem Depth_ms (0.1, 0.8, 1, etc.)
...
## replace name of Depth_m with the selected layer Depth_ms for ease of merging later
layer1$Depth_m <- 0.1 
layer2$Depth_m <- 0.8
layer3$Depth_m <- 1.6 
layer4$Depth_m <- 2.8 
...
write.csv(df.final, "./historical_model_selection/Data/Fluoro_FCR50_2014_2017.csv")
